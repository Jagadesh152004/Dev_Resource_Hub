    ...
    <div className="flex h-screen overflow-hidden bg-gray-950 text-gray-300 font-sans selection:bg-sky-500/30 selection:text-sky-200">
      ...
      <aside className="w-full md:w-64 md:shrink-0 border-b md:border-b-0 md:border-r border-white/10 bg-gray-900/50 backdrop-blur-xl flex flex-col absolute md:relative z-20 h-auto md:h-screen">
        ...
        <nav className="flex md:flex-col overflow-x-auto md:overflow-visible px-4 py-4 gap-2 no-scrollbar">
          <Link
            to="/"
            className="shrink-0 flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl font-medium transition-all"
          >
          ...
          <a
            href="#"
            className="shrink-0 flex items-center gap-3 px-4 py-3 bg-sky-500/10 text-sky-400 rounded-xl font-medium border border-sky-500/20 transition-all"
          >
            ...
            {filteredResources.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredResources.map((resource) => (
                  ...
                  <div
                    key={resource.id}
                    className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex flex-col hover:-translate-y-1 hover:border-sky-500/50 transition-all duration-300 shadow-xl group"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm text-gray-400 mb-6 bg-gray-900/50 p-4 rounded-xl border border-gray-800/50">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate" title={resource.channelName}>
                          {resource.channelName}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <User className="w-4 h-4 text-emerald-400" />
                        <span className="truncate" title={resource.authorName}>
                          {resource.authorName}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
